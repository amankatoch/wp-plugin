...
ob_start();
/*
Plugin Name: BriteWorx HTML5 RSS Audio Player
Plugin URI: http://www.briteworxdigital.com/rss-audio-player
...
Version: 1.0
Author URI: http://www.briteworxdigital.com
*/
...
{	
	add_menu_page( 'rssplayer', 'RSS Audio Player','manage_options', 'rssplayer','rssplayer_dashboard' ,plugin_dir_url( __FILE__ ) .'images/PodcastIcon16x16.png'); 
	add_submenu_page( 'rssplayer', 'Add RSS', 'Add New RSS Feed', 'manage_options', 'Add_new_rssplayer', 'add_new_rss'); 
	add_submenu_page( 'rssplayer', 'Add RSS', 'Feed List', 'manage_options', 'list_rssplayer', 'rssplayer_options');
	add_submenu_page( 'rssplayer', 'Settings', 'Settings', 'manage_options', 'setting_rss_player', 'setting'); 
	add_submenu_page( 'rssplayer', 'rssplayer', '', 'manage_options', 'edit_rssplayer', 'edit_rssplayer'); 
}

function edit_rssplayer()
{
	include('edit_rss.php');
}
...
{
	include('display_rss.php');
}

function add_new_rss()
{
	include('add_new_rssplayer.php');
}


function setting()
{
	include("setting_rss_player.php");
}
...

{
	...
	{
		$short .="<h2> <img src=".get_post_meta( '1', 'rss_player_title_icon', true ).">";
	}
	else
	{
	$short .="<h2> <img src=".plugin_dir_url( __FILE__ )."images/title.jpg>";
	}
...

	{
		...
		$short .='<a href="#pop1'.$xmlid.'" id="popUpA"><img class="email" src="'.plugin_dir_url( __FILE__ ).'images/email.png" border=0></a>
        <div class="pop-up" id="pop1'.$xmlid.'">
        <!-- The pop-up block -->
        <div class="popBox">
          <!-- If the content becomes larger than the pop-up this div will scroll the content -->
          <div class="popScroll" id="viewJob'.$xmlid.'">
          ';
          $short .="<form id='mail_form' name='mail_form' method='post' action='' style='padding:0px;margin:0px;'>
