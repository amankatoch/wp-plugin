...
function ModalPopupWindow() {
    var strOverLayHTML = '<div id="bailwal_div_overlay" onmousemove="bailwal_modalPopupWindow.DoDragging(event)" onmouseup="bailwal_modalPopupWindow.StopDragging()" style="position:absolute;z-index:9999999999; background-color:#000; filter: alpha(opacity = 70);opacity:0.7;"></div><div id="bailwal_div_frame_parent" style="position:absolute;z-index:99999999999; display:none;background-color:white;border:1px solid;"><table width="100%"  onmouseup="bailwal_modalPopupWindow.StopDragging()"><tr style="background-color:white;" id="bailwal_tr_title" onmousedown="bailwal_modalPopupWindow.StartDragging(event,this)" onmousemove="bailwal_modalPopupWindow.DoDragging(event)" onmouseup="bailwal_modalPopupWindow.StopDragging()"><span id="bailwal_span_title" style="color:White;font-size:9pt;padding-left:2px"></span><span id="bailwal_span_overlay_window_max" style="display:none" onclick="Maximisze();" style="cursor:pointer">max</span><span onclick="bailwal_modalPopupWindow.HideModalPopUp();" style="cursor:pointer"> <img id="bailwal_img_overlay_close"  src="http://www.iconarchive.com/download/i85560/graphicloads/100-flat/close.ico" alt="" style="position: relative; right: 5px;"> </span><td colspan="2" align="center" width="100%" id="bailwal_td_overlay"><div id="bailwal_div_message" style="margin-top:20px;display:none;"  ><span id="bailwal_span_message" style="padding-left:40px;padding-top:10px;padding-bottom:10px; font-size:9pt;"  ></span></div><span id="bailwal_span_loading"><img id="bailwal_img_overlay_loading" src="" alt="Loading..." /></span><iframe onload="bailwal_modalPopupWindow.OnUrlLoaded()" name="bailwal_overlay_frame" id="bailwal_overlay_frame" src="/Blank.htm" frameborder="0" scrolling="auto" ></iframe> </td></tr><tr id="bailwal_tr_overlay_btn" ><td colspan="2" align="center" style="padding-top:20px;padding-bottom:10px;"><input type="button" id="baiwlal_btn_overlay_1" onclick="bailwal_modalPopupWindow.HideModalPopUp()" value="Ok" style="display:none;padding:3px; border:1 solid #dcdcdc;BACKGROUND-COLOR: #f5f5f5;"   /><input type="button" id="bailwal_btn_overlay_2" onclick="bailwal_modalPopupWindow.HideModalPopUp()" value="Cancel" style="padding:3px;margin-left:20px;border:1 solid #dcdcdc;BACKGROUND-COLOR: #f5f5f5;"   /> </td></tr></table></div>';
    var orginalHeight;
    var orginalWidth;
    var btnStyle="";
    var maximize = false;
    ...
    function __InitModalPopUp(height, width, title) {
        ...
        divFrameParent.style.top = top-200 + "px";
        divFrameParent.style.left = left + "px";
        divFrameParent.style.height = height + "px";
        divFrameParent.style.width = width + "px";
...
function ShowMessage(con) {
    //alert(con);
 modalWin.ShowMessage("<p style='color:##007095;'>"+con+"</p>",130,400,'');
 }

function ShowMessage_Form(id) {
    // alert(id);
 ...
 modalWin.ShowMessage("<div id='viewJob"+id+"'><form class='form-table' id='mail_form' name='mail_form' method='post' action='' style='padding:0px;margin:0px;position: relative; bottom: 5px;left:-6px'>"+
        ...
        "<input style='width:300px;margin:0px;' type='text' name='fromEmail' id='fromEmail"+id+"'>"+
        "</td>"+
        "</tr>"+     
        "<tr>"+
        "<td align='right' style='padding-top:5px;'>Recipient Email:</td>"+ 
        "<td align='left' style='padding-top:5px;padding-left:5px;''>"+
        "<input style='width:300px;margin:0px;' type='text' name='toEmail' id='toEmail"+id+"'></td>"+
        "</tr>"+
        "<tr>"+
        "<td align='right' style='padding-top:5px;'>Subject:</td>"+ 
        "<td align='left' style='padding-top:5px; padding-left: 5px;'>"+
        "<input style='width:300px; margin: 0px;' type='text' name='subject' id='subject"+id+"' value='Check this out!'></td>"+
        "</tr>"+
        "<tr>"+
        "<td align='right' valign='top' style='padding-top:5px;'>Comments:</td>"+
        "<td align='left' style='padding-top:5px; padding-left: 5px;'>"+
        "<textarea id='briteWrox_comment"+id+"' class='textBody' style='width:300px;height:50px;padding:0px;overflow:auto;' name='comments'></textarea></td>"+
        "</tr>"+
        "<tr>"+    
        "<td></td>"+
        "<td align='left' style='padding-top:5px;padding-left:5px;'>"+
        "<input id='mail_link"+id+"' type='hidden' value='"+id+"' style='margin:0px;'>"+
        "<input type='hidden' name='hide_email' value="+url+" id='plugUrl"+id+"'/>"+
        "<input type='hidden' name='shareUrl' value='"+link+"'/ >"+
        "<input class='mailSend button' onclick='sendmail("+id+")' type='button' value='Send' style='margin:0px;'>"+
        "</td>"+
        "</tr>"+
        "</tbody>"+
        "</table>"+
...
function myFunction(val) {
    var myWindow = window.open(val, "", "width=200, height=100");
}