...
class displetretsidx_Google_CurlIO implements displetretsidx_Google_IO {
   ...
   * @return displetretsidx_Google_HttpRequest The resulting HTTP response including the
   * responseHttpCode, responseHeaders and responseBody.
   */
  public function authenticatedRequest(displetretsidx_Google_HttpRequest $request) {
    $request = displetretsidx_Google_Client::$auth->sign($request);
   ...
   *
   * @param displetretsidx_Google_HttpRequest $request the http request to be executed
   * @return displetretsidx_Google_HttpRequest http request with the response http code, response
   * headers and response body filled in
   * @throws displetretsidx_Google_IOException on curl or IO error
   */
  public function makeRequest(displetretsidx_Google_HttpRequest $request) {
    // First, check to see if we have a valid cached version.
    $cached = $this->getCachedRequest($request);
    if ($cached !== false) {
      if (displetretsidx_Google_CacheParser::mustRevalidate($cached)) {
        $addHeaders = array();
    ...
    if ($curlErrorNum != CURLE_OK) {
      throw new displetretsidx_Google_IOException("HTTP Error: ($respHttpCode) $curlError");
    }
   ...
   */
  public function setCachedRequest(displetretsidx_Google_HttpRequest $request) {
    // Determine if the request is cacheable.
    if (displetretsidx_Google_CacheParser::isResponseCacheable($request)) {
      displetretsidx_Google_Client::$cache->set($request->getCacheKey(), $request);
      return true;
    }
   ...
   * @visible for testing.
   * @param displetretsidx_Google_HttpRequest $request
   * @return displetretsidx_Google_HttpRequest|bool Returns the cached object or
   * false if the operation was unsuccessful.
   */
  public function getCachedRequest(displetretsidx_Google_HttpRequest $request) {
    if (false == displetretsidx_Google_CacheParser::isRequestCacheable($request)) {
      false;
    }

    return displetretsidx_Google_Client::$cache->get($request->getCacheKey());
  }
