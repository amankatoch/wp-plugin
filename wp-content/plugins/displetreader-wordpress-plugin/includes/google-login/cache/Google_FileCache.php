 ...
 */
class displetretsidx_Google_FileCache extends displetretsidx_Google_Cache {
  private $path;

  public function __construct() {
    global $displetretsidx_apiConfig;
    $this->path = $displetretsidx_apiConfig['ioFileCache_directory'];
  }
  ...
  private function createLock($storageFile) {
    ...
    if (! is_dir($storageDir)) {
      ...
      if (! @mkdir($storageDir, 0755, true)) {
        ...
        if (! is_dir($storageDir)) {
          throw new displetretsidx_Google_CacheException("Could not create storage directory: $storageDir");
        }
      }
  ...
  public function set($key, $value) {
    ...
    if (! @file_put_contents($storageFile, $data)) {
      $this->removeLock($storageFile);
      throw new displetretsidx_Google_CacheException("Could not store data in the file");
    }
    $this->removeLock($storageFile);
  }
  ...
  public function delete($key) {
    ...
    if (! @unlink($file)) {
      throw new displetretsidx_Google_CacheException("Cache file could not be deleted");
    }
  }
}
