 ...
 *
 */
class displetretsidx_Google_ServiceResource {
  ...
  /** @var displetretsidx_Google_Service $service */
  private $service;
  ...
  public function __call($name, $arguments) {
    ...
    foreach ($parameters as $key => $val) {
      if ($key != 'postBody' && ! isset($method['parameters'][$key])) {
        throw new displetretsidx_Google_Exception("($name) unknown parameter: '$key'");
      }
    }
    if (isset($method['parameters'])) {
      foreach ($method['parameters'] as $paramName => $paramSpec) {
        if (isset($paramSpec['required']) && $paramSpec['required'] && ! isset($parameters[$paramName])) {
          throw new displetretsidx_Google_Exception("($name) missing required param: '$paramName'");
        }
    ...
    if (isset($method['mediaUpload'])) {
      $media = displetretsidx_Google_MediaFileUpload::process($postBody, $parameters);
      if ($media) {
    ...
    $url = displetretsidx_Google_REST::createRequestUri($servicePath, $method['path'], $parameters);
    $httpRequest = new displetretsidx_Google_HttpRequest($url, $method['httpMethod'], null, $postBody);
    if ($postBody) {
      ...
      if (isset($contentType) && $contentType) {
        ...
        $contentTypeHeader['content-length'] = displetretsidx_Google_Utils::getStrLen($postBody);
      }
      $httpRequest->setRequestHeaders($contentTypeHeader);
    }

    $httpRequest = displetretsidx_Google_Client::$auth->sign($httpRequest);
    if (displetretsidx_Google_Client::$useBatch) {
      return $httpRequest;
    }

    // Terminate immediately if this is a resumable request.
    if (isset($parameters['uploadType']['value'])
        && displetretsidx_Google_MediaFileUpload::UPLOAD_RESUMABLE_TYPE == $parameters['uploadType']['value']) {
      $contentTypeHeader = array();
      if (isset($contentType) && $contentType) {
        $contentTypeHeader['content-type'] = $contentType;
      }
    ...
    return displetretsidx_Google_REST::execute($httpRequest);
  }

  public  function useObjects() {
    global $displetretsidx_apiConfig;
    return (isset($displetretsidx_apiConfig['use_objects']) && $displetretsidx_apiConfig['use_objects']);
  }
