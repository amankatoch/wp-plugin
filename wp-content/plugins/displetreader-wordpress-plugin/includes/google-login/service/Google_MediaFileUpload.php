 ...
 *
 */
class displetretsidx_Google_MediaFileUpload {
  ...
  public static function getUploadType($meta, &$payload, &$params) {
    if (isset($params['mediaUpload'])
        && get_class($params['mediaUpload']['value']) == 'displetretsidx_Google_MediaFileUpload') {
  ...
  public function nextChunk(displetretsidx_Google_HttpRequest $req, $chunk=false) {
    ...
    $httpRequest = new displetretsidx_Google_HttpRequest($this->resumeUri, 'PUT', $headers, $chunk);
    $response = displetretsidx_Google_Client::$io->authenticatedRequest($httpRequest);
    ...
    if (308 == $code) {
      ...
      return displetretsidx_Google_REST::decodeHttpResponse($response);
    }
  }
  ...
  private function getResumeUri(displetretsidx_Google_HttpRequest $httpRequest) {
    ...
    if ($body) {
      $httpRequest->setRequestHeaders(array(
        ...
        'content-length' => displetretsidx_Google_Utils::getStrLen($body),
